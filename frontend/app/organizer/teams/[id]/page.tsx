import {
  fetchOrganizerTeamByIdAction,
  updateOrganizerTeamAction,
} from "@/lib/actions/organizer-team-actions";
...
export default async function Page({
  ...
  const teamResponse = await fetchOrganizerTeamByIdAction(id);
  ...
  const boundAction = updateOrganizerTeamAction.bind(null, id);
  ...
  return (
    <div>
      ...
      <div className="mt-6 bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
        <OrganizerTeamForm team={teamResponse.data} action={boundAction} />
      </div>
    </div>
  );
}
