...
import { fetchMatchesAction } from "@/lib/actions/admin-match-actions";
...
export default async function Page({
  ...
  const res = await fetchMatchesAction({ page, size, search });
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage all scheduled matches.</p>
        </div>
        <Link
      ...
      <div className="mt-5">
        <SearchBar placeholder="Search title, venue, or city…" defaultValue={search} />
      </div>

      {!res.success || !res.data ? (
        <div className="mt-5 text-sm text-red-700 bg-red-50 border border-red-100 rounded-lg px-4 py-3">
          {res.message || "Failed to load matches"}
        </div>
      ) : (
        <div className="mt-5 bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          <MatchesTable matches={res.data.matches} />
          <div className="border-t border-gray-100 px-4">
            <Pagination basePath="/admin/matches" pagination={res.data.pagination} search={search} />
          </div>
        </div>
      )}
    </div>
